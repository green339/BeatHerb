...
const MusicWave = forwardRef(({}, ref) => {
  ...
  const fastForwardBtn = () => {
    eeRef.current.emit("fastforward");
  };
  const recordBtn = () => {
    eeRef.current.emit("record");
  };

  const audiorenderingfinished = (type, data) => {
    console.log("audiorndering");
  ...
  const downloadBtn = () => {
    console.log("aaa");
    eeRef.current.emit("startaudiorendering", "wav");
    eeRef.current.on("audiorenderingfinished", audiorenderingfinished);
    console.log(eeRef.current);
  };
  const trimBtn = () => {
  ...
  return (
    <div className="flex w-full">
      <div className="w-1/12 bg-base-200 border-r-2 border-b-2 border-gray-300">
        <svg
      ...
      <div
        className="w-11/12 bg-base-200 border-b-2 border-gray-300"
